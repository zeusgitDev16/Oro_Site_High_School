...
import 'package:oro_site_high_school/screens/student/dialogs/student_compose_message_dialog.dart';

/// Student Messages Screen - Full messaging interface with folders, labels, and threads
/// Follows the same architecture as Teacher/Admin messaging system
...
class _StudentMessagesScreenState extends State<StudentMessagesScreen> {
  late final StudentMessagesState state;

  @override
  void initState() {
    super.initState();
    state = StudentMessagesState()..initMockData();
  }
          ...
          actions: [
            IconButton(
              ...
              onPressed: () {
                showDialog(
                  context: context,
                  builder: (context) => StudentComposeMessageDialog(state: state),
                );
              },
  ...
  Widget _buildLeftSidebar() {
    return Container(
      ...
      child: Consumer<StudentMessagesState>(
  ...
  Widget _buildThreadList() {
    return Consumer<StudentMessagesState>(
      builder: (context, state, _) {
        return Column(
          children: [
            // Search bar
  ...
  Widget _buildThreadItem(Thread thread, bool isSelected, StudentMessagesState state) {
  ...
  Widget _buildMessageHeader(Thread thread, StudentMessagesState state) {
    final otherParticipants = thread.participants
        .where((p) => p.id != 's1')
        .map((p) => p.name)
        .join(', ');
  ...
  Widget _buildMessageBubble(Msg message) {
    final isMe = message.author.id == 's1';
  ...
  Widget _buildMessageComposer(StudentMessagesState state) {
